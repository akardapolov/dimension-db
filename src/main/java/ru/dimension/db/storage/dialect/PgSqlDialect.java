...
public class PgSqlDialect implements DatabaseDialect {
  ...
  public String getSelectClassStackedPercentile(PercentileFunction percentileFunction,
    ...
    return "SELECT PERCENTILE_DISC(" + percentileFunction.getValue()
        + ") WITHIN GROUP (ORDER BY " + colName + ") AS value ";
  }
    ...
    for (String value : filterData) {
      ...
      if (value == null || value.trim().isEmpty()) {
        ...
        if (compareFunction == CompareFunction.CONTAIN) {
          conditionBuilder.append("LOWER(").append(columnName).append(") ILIKE '%")
    ...
    for (String filterValue : filterData) {
      ...
      if (filterValue == null || filterValue.trim().isEmpty()) {
        ...
        if (CompareFunction.CONTAIN.equals(compareFunction)) {
          ...
          condition = "LOWER(" + columnName + ") ILIKE '" + formattedValue.replace("'", "''").replace("\\", "\\\\") + "'";
        } else {
