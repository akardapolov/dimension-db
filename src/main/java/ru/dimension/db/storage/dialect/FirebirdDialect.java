...
public class FirebirdDialect implements DatabaseDialect {
  ...
  public String getSelectClassGantt(CProfile firstCProfile, CProfile secondCProfile) {
    ...
    // Fix: Quote the "value" alias as it's a reserved keyword in Firebird.
  ...
  public String getWhereClass(CProfile tsCProfile,
    ...
    if (DataType.DATE.equals(dataType) ||
      ...
      return "WHERE " + tsCProfile.getColName()
  ...
  private String buildCondition(FilterCondition condition) {
    ...
    String columnName = filterProfile.getColName();
    ...
    for (String value : filterData) {
      ...
      if (value == null || value.trim().isEmpty()) {
        ...
        if (compareFunction == CompareFunction.CONTAIN) {
          // Firebird uses LIKE with % for pattern matching
          conditionBuilder.append("UPPER(").append(columnName).append(") LIKE '%")
              .append(escapedValue.toUpperCase()).append("%'");
        } else {
    ...
    for (String filterValue : filterData) {
      ...
      if (filterValue == null || filterValue.trim().isEmpty()) {
        ...
        if (CompareFunction.CONTAIN.equals(compareFunction)) {
          ...
          condition = "UPPER(" + columnName + ") LIKE '" + formattedValue.replace("'", "''") + "'";
        } else {
